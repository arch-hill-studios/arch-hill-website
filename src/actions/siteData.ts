...
import type { FOOTER_QUERY_RESULT, HEADER_QUERY_RESULT, SEO_META_DATA_QUERY_RESULT, BUSINESS_CONTACT_INFO_QUERY_RESULT, COMPANY_LINKS_QUERY_RESULT, CONTACT_FORM_SETTINGS_QUERY_RESULT, LEGAL_PAGES_VISIBILITY_QUERY_RESULT, CONTACT_GENERAL_CONTENT_QUERY_RESULT, CONTACT_CONFIRMATION_EMAIL_QUERY_RESULT } from '@/sanity/types';
...
export async function getContactGeneralContent(fetchFn: FetchFn = staticSanityFetch): Promise<CONTACT_GENERAL_CONTENT_QUERY_RESULT | null> {
  const { data } = await fetchFn({
    query: CONTACT_GENERAL_CONTENT_QUERY,
    tags: ['sanity', 'contactGeneralContent'],
  });

  return data as CONTACT_GENERAL_CONTENT_QUERY_RESULT | null;
}
